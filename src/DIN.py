...
class DIN(BaseModel):
    ...
    def forward(self, inputs):
        ...
        target_emb = item_feat_emb[:, -1, :] # [batch_size, embedding_dim]
        sequence_emb = item_feat_emb[:, 0:-1, :] # [batch_size, sequence_length-1, embedding_dim]
        pooling_emb = self.attention_layers(target_emb, sequence_emb, mask)
        emb_list += [target_emb, pooling_emb]
        feature_emb = torch.cat(emb_list, dim=-1) # [batch_size, total_embedding_dim]
